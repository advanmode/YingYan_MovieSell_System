    ...
    <div>
        ...
        <div class="whole">
            <div class="left">
                ...
                <div class="panel">
                    <div class="panel-header">
                        <h2 style="color: rgb(255,180,0)">经典影片</h2>
            ...
            <div class="right">
                <div class="panel">
                    <div class="panel-header">
                        <h2 style="color: #ffb400">TOP 99</h2>
                        <a href="/rankingList/top100List">完整榜单</a>
                    </div>
                    <div class="panel-content">
                        <div class="board" v-for="(item, index) in top100List" :key="index">
                            ...
                            <div class="board-middle">
                                <a :href="'/movieInfo/' + item.movieId">
                                    <p class="name">{{ item.movieNameCn }}</p>
                                </a>
                            </div>
                ...
                <div class="panel">
                    <div class="panel-header">
                        <h2 style="color: #ffb400">热门演员</h2>
                    </div>
                    <div class="panel-content">
                        <div class="board" v-for="(item, index) in actorList" :key="index">
                            <div class="board-left">
                                <i class="board-index">{{index+1}}</i>
                            </div>
                            <div class="board-middle">
                                <a :href="'/actor/' + item.actorId">
                                    <p class="name">{{ item.actorName }}</p>
                                </a>
                            </div>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        ...
        async getTop100List() {
            const { data: resp } = await axios.get("sysMovie/rankingList/4", {
        ...
        async getActorList() {
            const { data: resp } = await axios.get("sysActor", {
                params: this.queryInfo4,
            });
            console.log(resp);
            if (resp.code != 200) return this.$message.error(resp.msg);
    ...
    // computed: {
    //     upcomingMovieList() {
    //         return this.classicMovieList.slice(0, 5);
    //     },
    //     ongoingMovieList() {
    //         return this.classicMovieList.slice(6, 10);
    //     },
    //     classicMovieListI() {
    //         return this.classicMovieList.slice(10, 16);
    //     },
    // },
};
</script>
