...
export default {
    ...
    watch: {
        movieItem() {
            this.movieItem.moviePoster = JSON.parse(
                this.movieItem.moviePoster
            )[0];
            this.movieItem.moviePictures = JSON.parse(
                this.movieItem.moviePictures
            );
            this.movieItem.releaseDate = moment(
                this.movieItem.releaseDate
            ).format("YYYY-MM-DD");
            const tmp = this.movieItem.majorActorNameList;
            this.movieItem.majorActorNameList = tmp
                .slice(0, Math.min(tmp.length, 3))
