        ...
        <el-table-column label="操作" width="150">
          <template slot-scope="scope">
            <el-tooltip effect="dark" content="修改电影年代" placement="top" :enterable="false" :open-delay="500">
              <el-button v-has type="primary" icon="el-icon-edit" size="mini"
                         @click="showEditDialog(scope.row.movieAgeId)"></el-button>
            </el-tooltip>
            <el-tooltip effect="dark" content="删除年代" placement="top" :enterable="false" :open-delay="500">
              <el-button v-has type="danger" icon="el-icon-delete" size="mini"
                         @click="deleteMovieAgeById(scope.row.movieAgeId)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    getMovieAgeList() {
      const _this = this;
      axios.get('sysMovieAge', {params: _this.queryInfo}).then(resp => {
        console.log(resp)
    ...
    addMovieAge() {
      ...
      this.$refs.addFormRef.validate(async valid => {
        ...
        await axios.post('sysMovieAge', JSON.stringify(_this.addForm)).then(resp => {
          console.log(resp)
    ...
    showEditDialog(id) {
      const _this = this
      axios.get('sysMovieAge/' + id).then(resp => {
        console.log(resp)
    ...
    editMovieAgeInfo() {
      this.$refs.editFormRef.validate(async valid => {
        ...
        await axios.put('/sysMovieAge', JSON.stringify(_this.editForm)).then(resp => {
          if (resp.data.code !== 200) {
    ...
    async multipleDelete() {
      ...
      await axios.delete('sysMovieAge/' + ids).then(resp => {
        if (resp.data.code !== 200) {
    ...
    async deleteMovieAgeById(id) {
      ...
      await axios.delete('sysMovieAge/' + id).then(resp => {
        if (resp.data.code !== 200) {
