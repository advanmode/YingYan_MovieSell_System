          ...
          <template slot-scope="scope">
            <el-tooltip effect="dark" content="修改演员角色" placement="top" :enterable="false" :open-delay="500">
              <el-button type="primary" icon="el-icon-edit" size="mini"
                         @click="showEditDialog(scope.row.actorRoleId)"></el-button>
            </el-tooltip>
            <el-tooltip effect="dark" content="删除角色" placement="top" :enterable="false" :open-delay="500">
              <el-button type="danger" icon="el-icon-delete" size="mini"
                         @click="deleteActorRoleById(scope.row.actorRoleId)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    getActorRoleList() {
      const _this = this;
      axios.get('sysActorRole', {params: _this.queryInfo}).then(resp => {
        console.log(resp)
    ...
    addActorRole() {
      ...
      this.$refs.addFormRef.validate(async valid => {
        ...
        await axios.post('sysActorRole', JSON.stringify(_this.addForm)).then(resp => {
          console.log(resp)
    ...
    showEditDialog(id) {
      const _this = this
      axios.get('sysActorRole/' + id).then(resp => {
        console.log(resp)
    ...
    editActorRoleInfo() {
      this.$refs.editFormRef.validate(async valid => {
        ...
        await axios.put('sysActorRole', JSON.stringify(_this.editForm)).then(resp => {
          if (resp.data.code !== 200) {
    ...
    async multipleDelete() {
      ...
      await axios.delete('sysActorRole/' + ids).then(resp => {
        if (resp.data.code !== 200) {
    ...
    async deleteActorRoleById(id) {
      ...
      await axios.delete('sysActorRole/' + id).then(resp => {
        if (resp.data.code !== 200) {
